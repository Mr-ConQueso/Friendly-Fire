Shader "PostProcessing/Blur"
{
    ...
    SubShader
    {
		...
		CBUFFER_START(UnityPerMaterial)
			...
			uint _GridSize;
			uint _BlurMode; // 0 for gaussian, 1 for boxed
			float _Spread;
		CBUFFER_END
		...
		float boxBlur()
        {
            // For Box Blur, each pixel in the kernel has the same weight
            return 1.0 / _GridSize;
        }
        ...
        Pass
        {
            ...
            float4 frag_horizontal (v2f i) : SV_Target
			{
				...
				{
					float gauss = (_BlurMode == 0) ? gaussianBlur(x) : boxBlur();
					gridSum += gauss;
		...
		Pass
        {
            ...
            float4 frag_vertical (v2f i) : SV_Target
			{
				...
				{
					float gauss = (_BlurMode == 0) ? gaussianBlur(y) : boxBlur();
					gridSum += gauss;
